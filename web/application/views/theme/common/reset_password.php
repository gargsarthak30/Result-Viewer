...
<div class="container">
						...
						<div class="row">
							...
							<center><b>Username :</b> <?=$username?></center><br>
								<form id="login-form" action="<?=site_url('password/reset_form').'/'.$username;?>" method="post" role="form" style="display: block;">
									...
									<div class="form-group">
										<input type="password" name="conf_pass" id="pass" tabindex="2" class="form-control" placeholder="Confirm Password" value="" required>
									</div>
									<div class="form-group">
										<div class="row">
	...
	<?php
    $flash = $this->session->flashdata('conf_mail').$this->session->flashdata('verify_link');
    if(!empty($flash))
    {
?>
        <script>
